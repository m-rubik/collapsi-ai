...
def train_dqn_agent(episodes=1000):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            state_vec = encode_state(game, agent_name='P1')
            ...
            if current_player.name == 'P1' and current_player.active:
                ...
                if not legal_moves:
                    current_player.active = False
                    next_state_vec = encode_state(game, agent_name='P1')
                    agent.remember(state_vec, 0, -10.0, next_state_vec, True)
                    done = True
                    continue
                ...
                if action_idx not in legal_action_indices:
                ...
                reward = 0.1
                next_state_vec = encode_state(game, agent_name='P1')
                agent.remember(state_vec, action_idx, reward, next_state_vec, False)
                agent.update_model()
            ...
            if len(alive) == 1:
                ...
                final_state_vec = encode_state(game, agent_name='P1')
                agent.remember(state_vec, action_idx, final_reward, final_state_vec, True)
        ...
        if episode % 1000 == 0:
            print(f"Episode {episode}: Wins so far = {wins}, Epsilon = {agent.epsilon:.3f}")
